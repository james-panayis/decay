...
#THIIS NEEDS TO CHANGE!!!!!!!!!!!!!!!#
BcSignalSel = "(B_BKGCAT == 20 || B_BKGCAT == 40) && abs(hadron_TRUEID) == 211"
...
if __name__ == '__main__':
    ...
    # && survive_trigger_selection"
    sig_selection = f"{baseline_selection} && {BcSignalSel}"
    ...
    if not doesXGBInputExist(data, year, polarity, recalculate=False):
        ...
        sig_sample_name = "MCBc_pimumu_2016_up"
        bkg_sample_name = "pimumu_2016_up"

        sig_file_name = cd.samples[sig_sample_name]["ntuples"]
        bkg_file_name = cd.samples[bkg_sample_name]["ntuples"]

        sig_chain = ROOT.TChain("pimumu/DecayTree")
        sig_chain.Add(sig_file_name)

        bkg_chain = ROOT.TChain("pimumu/DecayTree")
        ...
        train_data = pd.concat([
            sig_frame[columns + ["abs(muplus_P-muminus_P)"]],
            bkg_frame[columns + ["abs(muplus_P-muminus_P)"]]
        ],
                               sort=False,
                               ignore_index=True)
...
##Train without interface
#    dtrain = xgb.DMatrix(X_train[input_columns + ["deltaP"]] , X_train[["label"]])
#
#    dtest = xgb.DMatrix(X_test[input_columns + ["deltaP"]] , X_test[["label"]])
#
#    model = training_func_direct(dtrain)
#    model.save_model(f'Bc_jpsipi_{year}_{polarity}_xgboost_model_old.json')
#
#    pred_train = model.predict(dtrain)
#    pred_test = model.predict(dtest)
#
#    X_train["xgb_output"] = pred_train
#    X_test["xgb_output"] = pred_test
#
#    X_train_dict = X_train.to_dict("list")
#    X_train_dict = {
#        key: np.array(X_train_dict[key])
...
#
#    #save file
#    df_train.Snapshot('tree', f'{data}_{year}_{polarity}_xgboost_train_old.root')
#    df_test.Snapshot('tree', f'{data}_{year}_{polarity}_xgboost_test_old.root')
#
#
#-----------------------------------------------------------------#

#    print("max: " , X_test["max(muplus_PT,muminus_PT)"] , "muplus: " , X_test["muplus_PT"] , "muminus: " , X_test["muminus_PT"])
#
#    name = f"{data}_{year}_{polarity}_xgboost_"
#    model = train_xgb_classifier(
#        X_train[input_columns + derived_inputs],
#        X_test[input_columns + derived_inputs], y_train, y_test,
#        X_train["mva_mcweight_nTracks_B_PT"],
#        X_test["mva_mcweight_nTracks_B_PT"], f"{name}")
#
#    Xtest, Xtrain = predict_and_save(
#        model, X_test, X_train, input_columns + derived_inputs, "xgb_output",
...
#
#    plot_name = "new_vars_top10_noVTXISO_nocos"
#
...
#                     f"{plot_name}")
#
#draw_correlation(Xtest.loc[Xtest.label == 1][input_columns + derived_inputs + more_columns], 'Correlations',
#                 'input_correlations_signal.pdf')

#draw_correlation(Xtest.loc[Xtest.label == 0][input_columns + derived_inputs + more_columns], 'Correlations',
#                 'input_correlations_bkg.pdf')
...
# ---------- To plot functions from the cache file for mva_xgboost script outputs --------
    ...
    #        f"{from_cache}run2_Bc_jpsipi_test.root",
    #        f"{from_cache}run2_Bc_jpsipi_train.root", "tree", "tree")

    Xtest1, Xtrain1 = load_test_train(
        f"reflected_Bc_jpsipi_2016_Up_xgboost__test.root",
        f"reflected_Bc_jpsipi_2016_Up_xgboost__train.root", "tree", "tree")
    print("INPUT TRAIN AND TEST LOADED")

    Xtest2, Xtrain2 = load_test_train(
        f"Bc_jpsipi_2016_Up_xgboost__test.root",
        f"Bc_jpsipi_2016_Up_xgboost__train.root", "tree", "tree")
    print("INPUT TRAIN AND TEST LOADED")

    #    Xtest3, Xtrain3 = load_test_train(
    #        f"{from_selections}xgboutput_tuple_pimumu_DecayTree_mcBsstpi_14113020_2016_MagUp.root",
    #        f"{from_selections}xgboutput_tuple_pimumu_DecayTree_mcBsstpi_14113020_2016_MagUp.root", "XGBoutput", "XGBoutput")
    #    print("INPUT TRAIN AND TEST LOADED")
    #
    #    Xtest4, Xtrain4 = load_test_train(
    #        f"{from_selections}xgboutput_tuple_pimumu_DecayTree_mcBstpi_14113021_2016_MagUp.root",
    #        f"{from_selections}xgboutput_tuple_pimumu_DecayTree_mcBstpi_14113021_2016_MagUp.root", "XGBoutput", "XGBoutput")
    #    print("INPUT TRAIN AND TEST LOADED")
    #
